#ifndef NEMO_CUDA_PLUGINS_IF_COND_ALPHA_CU
#define NEMO_CUDA_PLUGINS_IF_COND_ALPHA_CU
...
/*! \file IF_curr_alpha.cu Neuron update CUDA kernel for conductance-based
 * alpha decay integrate-and-fire neurons. */
	...
	nidx_dt* s_fired)    // s_NIdx, so can handle /all/ neurons firing
{
    ...
    const float* g_E_rev      = g_neuronParameters + PARAM_E_REV      * neuronParametersSize;
    const float* g_I_rev      = g_neuronParameters + PARAM_I_REV      * neuronParametersSize;
    ...
    const float* g_DIe0       = state<HISTORY_LENGTH, STATE_COUNT, STATE_DIE      >(cycle, s_params.pitch32, g_neuronState);
	const float* g_DIi0       = state<HISTORY_LENGTH, STATE_COUNT, STATE_DII      >(cycle, s_params.pitch32, g_neuronState);
    ...
    float* g_DIe1       = state<HISTORY_LENGTH, STATE_COUNT, STATE_DIE      >(cycle+1, s_params.pitch32, g_neuronState);
	float* g_DIi1       = state<HISTORY_LENGTH, STATE_COUNT, STATE_DII      >(cycle+1, s_params.pitch32, g_neuronState);
	...
	for(unsigned bNeuron=0; bNeuron < s_partitionSize; bNeuron += THREADS_PER_BLOCK) {
		...
		if(bv_isSet(neuron, s_valid)) {
			...
			g_DIe1[neuron] = (1.0f - 1.0f/g_tau_syn_E[neuron]) * g_DIe0[neuron] + g_currentE[neuron];
			g_DIi1[neuron] = (1.0f - 1.0f/g_tau_syn_I[neuron]) * g_DIi0[neuron] + g_currentI[neuron];

            float Ie  = g_Ie0[neuron] + (2.7182818284590451f * g_DIe1[neuron] - g_Ie0[neuron])/g_tau_syn_E[neuron];
    		float Ii  = g_Ii0[neuron] + (2.7182818284590451f * g_DIi1[neuron] - g_Ii0[neuron])/g_tau_syn_I[neuron];

            float v = g_v0[neuron];

			/* Update the incoming currents */
			float I = Ie * (g_E_rev[neuron] - v) + Ii * (g_I_rev[neuron] - v) + s_currentExt[neuron] + g_I_offset[neuron];
